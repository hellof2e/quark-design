...
const genaratorTags = () => {
	...
	for (let componentDir of componentDirs) {
		...
		if (stat.isDirectory()) {
			...
			componentTags[`quark-${componentDir}`] = { attributes: [] };
			for (let sourceMap of sourcesMap) {
				let propItem = sourceMap.filter((source) => source.type === "inline")
					.length
					? `${
							sourceMap.filter((source) => source.type === "inline")[0].content
					  }`
					: "";
				componentTags[`quark-${componentDir}`]["attributes"].push(propItem);
			}
		}
	}
		...
		if (stat.isDirectory()) {
			...
			for (let sourceMap of sourcesMap) {
				...
				const typeItem = inlineItem.length
					...
					: "";
				console.log(inlineItem[3], 2);
				const defaultItem = inlineItem.length
					? `${inlineItem[3]?.content}`
					: "";
				componentTags[`quark-${componentDir}/${propItem}`] = {
					type: `${typeItem}`,
...
const writeTags = () => {
	...
	const componentTagsPath = path.resolve(
		__dirname,
		"./../dist/smartips/tags.json"
	);
...
const writeAttributes = () => {
	...
	const distPath = path.resolve(__dirname, "./../dist");
	const componentAttributespPath = path.resolve(
		__dirname,
		"./../dist/smartips/attributes.json"
	);
	if (!fs.existsSync(path.join(distPath + "/smartips"))) {
		fs.mkdirSync(path.join(distPath + "/smartips"));
	}
