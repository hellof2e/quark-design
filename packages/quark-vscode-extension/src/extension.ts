    ...
    const text = components
      ...
      .map((item: string) => {
        ...
        return new vscode.MarkdownString(
          `[QuarkUI -> $(references) 请查看 ${bigCamelize(item)} 组件官方文档](${DOC}${site})\n`,
          true
        );
      });
...
const provideCompletionItems = () => {
  ...
  Object.keys(componentMap).forEach((key: string) => {
    ...
    completionItems.push(
      new vscode.CompletionItem(`quark-${key}`, vscode.CompletionItemKind.Field),
      new vscode.CompletionItem(bigCamelize(`quark-${key}`), vscode.CompletionItemKind.Field)
    );
  });
