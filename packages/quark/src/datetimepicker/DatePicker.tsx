...
import { clamp, getMonthEndDay, getTrueValue, isDate, padZero, times } from './utils'
...
class QuarkDatePicker extends QuarkElement {
  ...
  formatValue(value){
    if (isDate(value)) {
      const timestamp = clamp(value.getTime(), this.innerMinDate.getTime(), this.innerMaxDate.getTime());
  ...
  calcRanges() {
    const {
      ...
      maxHour,
      maxMinute
    } = this.getBoundary("max", this.currentDate || this.innerMinDate);
    ...
    const {
      ...
      minHour,
      minMinute
    } = this.getBoundary("min", this.currentDate || this.innerMinDate);
    let result = [
  ...
  setColumns() {
    ...
    if(this.currentDate) {
      let date = []
      date = [this.currentDate.getFullYear(), this.currentDate.getMonth() + 1,this.currentDate.getDate(), this.currentDate.getHours(), this.currentDate.getMinutes()];
      ...
      >
        {
          this.showtoolbar && <div slot="header" class='quark-date-picker-header'>
            ...
            <span class="quark-date-picker-confirm-btn" onclick={this.onConfirm}>
